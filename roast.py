...
from langchain.schema import Document  # <-- Added: Required for stuff chain
...
async def generate_roast(resume_text, candidate_name):
    ...
    # Split into chunks for consistency, but wrap as Document objects
    text_chunks = await get_text_chunks(resume_text)
    docs = [Document(page_content=chunk) for chunk in text_chunks]

    # Fallback: if no chunks (unlikely), use full text
    if not docs:
